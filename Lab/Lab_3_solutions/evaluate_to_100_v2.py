...
'''
Finds all possible ways of inserting + and - signs in the sequence 123456789
(at most one sign before any digit) such that the resulting arithmetic expression
evaluates to 100.
Does not use eval.
'''
...
def test(sign, i):
    ...
    for digit in range(2, 10):
        ...
        i //= 3
    if the_sum + sign * number == 100:
        return True
    return False


def print_solution(one_or_minus_one, i):
    print(f'{one_or_minus_one}', end = '')
    for digit in range(2, 10):
        one_of_3_options = i % 3
        if one_of_3_options % 2 == 0:
            print(' + ', end = '') if one_of_3_options else print(' - ', end = '')
        print(digit, end = '')
        i //= 3
    print(' = 100')
