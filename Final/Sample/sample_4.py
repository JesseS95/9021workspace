...
def get_words_and_word_relationships():
    dic_word = defaultdict(set)
    with open(dictionary_file) as f:
        for rows in f:
            rows = rows.strip()
            dic_word[len(rows)].add(rows)
    return dic_word
...
def is_word_word_ladder(word_1, word_2, candidate_ladder):
    ...
    if len(word_1) != len(word_2):
        return False
    if not word_1 in dic_word[len(word_1)] or not word_2 in dic_word[len(word_2)]:
        return False
    closest_dic = defaultdict(list)

    for w1 in dic_word[len(word_1)]:
        for w2 in dic_word[len(word_1)]:
            if close(w1, w2):
                closest_dic[w1].append(w2)
    for i in range(len(candidate_ladder) - 1):
        if not candidate_ladder[i+1] in closest_dic[candidate_ladder[i]]:
            return False
    final_length = 0
    Queue = deque()
    Queue.append([word_1])
    while len(Queue):
        one_path = Queue.pop()
        if one_path[-1] == word_2:
            final_length = len(one_path)
            break
        for w in closest_dic[one_path[-1]]:
            if w in one_path:
                continue
            Queue.appendleft(one_path + [w])
    if final_length == len(candidate_ladder):
        return True
    return False
...
if __name__ == '__main__':
    ...
    dic_word = get_words_and_word_relationships()
    import doctest
    doctest.testmod()
